...
def getIntermediateFusionDataset(cls):
    ...
    class IntermediateFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            # print("Initializing IntermediateFusionDataset...")
            super().__init__(params, visualize, train)
            
            # print("Setting supervise_single flag...")
            ...
            # print(f"supervise_single set to: {self.supervise_single}")
            
            # print("Setting proj_first flag...")
            self.proj_first = True
            self.proj_first = False if 'proj_first' not in params['fusion']['args']\
                                         else params['fusion']['args']['proj_first']
            # print(f"proj_first set to: {self.proj_first}")

            # print("Generating anchor box...")
            self.anchor_box, self.num_anchors_per_location = self.post_processor.generate_anchor_box()
            # import pdb; pdb.set_trace()  # Add this line
            # self.anchor_box_torch = torch.from_numpy(self.anchor_box)
            # print(f"Anchor box shape: {self.anchor_box.shape}")

            # print("Setting kd_flag...")
            self.kd_flag = params.get('kd_flag', False)
            # print(f"kd_flag set to: {self.kd_flag}")

            # print("Setting box_align...")
            self.box_align = False
            if "box_align" in params:
                # print("Box align parameters found, configuring...")
                self.box_align = True
                self.stage1_result_path = params['box_align']['train_result'] if train else params['box_align']['val_result']
                # print(f"Loading stage1 results from: {self.stage1_result_path}")
                self.stage1_result = read_json(self.stage1_result_path)
                self.box_align_args = params['box_align']['args']
                # print("Box align configuration complete")
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            label_dict = self.post_processor.generate_label(
                gt_box_center=object_bbx_center, anchors=self.anchor_box, num_anchors_per_location=self.num_anchors_per_location, mask=object_bbx_mask
            )
            ...
            if self.load_camera_file:
                ...
                # Use number of cameras to determine which config to use
                num_cameras = len(camera_data_list)
                modality = 'm4' if num_cameras == 2 else 'm2'
                data_aug_conf = self.params['heter']['modality_setting'][modality]['data_aug_conf']
        ...
        def __getitem__(self, idx):
            ...
            # print("object_stack first row:", object_stack[0])
            ...
            # print("object_bbx_center shape:", object_bbx_center.shape)
            # print("object_stack shape:", object_stack.shape)

            # Add padding column if object_stack only has 7 columns
            if object_stack.shape[1] == 7:
                object_stack = np.pad(object_stack, ((0,0), (0,1)), mode='constant')
                # print("After padding, object_stack shape:", object_stack.shape)

            object_bbx_center[:object_stack.shape[0], :] = object_stack
            mask[:object_stack.shape[0]] = 1
            
            if self.load_lidar_file:
                merged_feature_dict = merge_features_to_dict(processed_features, camera=False)
            ...
            all_anchors, num_anchors_per_location = self.anchor_box, self.num_anchors_per_location
        ...
        def collate_batch_train(self, batch):
            ...
            if self.load_lidar_file:
                merged_feature_dict = merge_features_to_dict(processed_lidar_list, camera=False)
            ...
            if self.kd_flag:
                ...
                merged_feature_proj_dict = merge_features_to_dict(processed_lidar_proj_list, camera=False)
        ...
        def post_process(self, data_dict, output_dict):
            ...
            pred_score : torch.Tensor
                The tensor of prediction scores after NMS.
            gt_box_tensor : torch.Tensor
                The tensor of ground truth bounding boxes.
            gt_label_tensor : torch.Tensor, optional
                The tensor of ground truth class labels. Only returned if self.class_names is not None.
            """
