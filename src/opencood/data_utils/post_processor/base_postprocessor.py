...
class BasePostprocessor(object):
    ...
    def generate_gt_bbx(self, data_dict):
        ...
        """
        # print("Starting generate_gt_bbx function")
        gt_box3d_list = []
        # used to avoid repetitive bounding box
        label_list = []
        object_id_list = []

        # print(f"Processing {len(data_dict)} CAVs")
        for cav_id, cav_content in data_dict.items():
            # print(f"Processing CAV {cav_id}")
            ...
            # print(f"Number of objects in CAV {cav_id}: {len(object_bbx_center)}")

            # print("Converting boxes to corners")
            object_bbx_corner = \
                box_utils.boxes_to_corners_3d(object_bbx_center,
                                              self.params['order'])
            # print("Projecting boxes to ego frame")
    ...
    def generate_object_center_v2xreal(self,
                               cav_contents,
                               reference_lidar_pose):
        """
