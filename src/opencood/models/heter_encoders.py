...
class LiftSplatShoot(nn.Module):
    ...
    def get_cam_feats(self, x):
        ...
        # print(f"Input shape: B={B}, N={N}, C={C}, imH={imH}, imW={imW}")
        ...
        # print(f"After camencode: {x.shape}")
    ...
    def get_voxels(self, x, rots, trans, intrins, post_rots, post_trans):
        # print("[DEBUG] get_voxels input shapes:")
        # print(f"[DEBUG] x: {x.shape}")
        # print(f"[DEBUG] rots: {rots.shape}")
        # print(f"[DEBUG] trans: {trans.shape}")
        # print(f"[DEBUG] intrins: {intrins.shape}")
        # print(f"[DEBUG] post_rots: {post_rots.shape}")
        # print(f"[DEBUG] post_trans: {post_trans.shape}")
        ...
        # print(f"[DEBUG] After get_geometry - geom shape: {geom.shape}")

        x_img, depth_items = self.get_cam_feats(x)  # 提取图像特征并预测深度编码 x: B x N x D x fH x fW x C(4 x N x 42 x 16 x 22 x 64)
        # print(f"[DEBUG] After get_cam_feats:")
        # print(f"[DEBUG] x_img shape: {x_img.shape}")
        # print(f"[DEBUG] depth_items type: {type(depth_items)}")

        x = self.voxel_pooling(geom, x_img)  # x: 4 x 64 x 240 x 240
        # print(f"[DEBUG] After voxel_pooling - x shape: {x.shape}")

        return x, depth_items

    def forward(self, data_dict, modality_name):
        # print(f"[DEBUG] Processing modality: {modality_name}")
        
        image_inputs_dict = data_dict[f'inputs_{modality_name}']
        # print(f"[DEBUG] Image inputs dict keys: {image_inputs_dict.keys()}")
        ...
        # print(f"[DEBUG] After get_voxels:")
        # print(f"[DEBUG] x shape: {x.shape}")
        # print(f"[DEBUG] depth_items type: {type(depth_items)}")
        
        if self.depth_supervision:
            # print("[DEBUG] Depth supervision enabled, storing depth items")
