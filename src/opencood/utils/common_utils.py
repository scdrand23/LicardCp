...
def update_dict(d1,d2):
    ...
    for i in d1:
        ...
        if isinstance(d1[i], dict):
            update_dict(d1[i],d2)
    return d1
def merge_features_to_dict(processed_feature_list, camera=True, merge=None):
    """
    Merge the preprocessed features from different cavs to the same
    dictionary, handling different camera counts properly.
    ...
    processed_feature_list : list
        ...
        different cavs.
    camera : bool
        Whether processing camera features
    merge : str
        "stack" or "cat". used for images
    ...
    if not camera:
        return merge_features_to_dict_lidar(processed_feature_list, merge)
...
def merge_features_to_dict_lidar(processed_feature_list, merge=None):
    """
    Merge the preprocessed features from different cavs to the same
    dictionary.
