...
class PointPillarLoss(nn.Module):
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        print(f"Number of positive samples: {positives.sum().item()}")
        print(f"Number of negative samples: {negatives.sum().item()}")

        # Rename variables
        print("Renaming prediction variables...")
        ...
        # Get classification predictions and reshape
        ...
        # print(self.pos_cls_weight.shape)
        cls_weights = positives * self.pos_cls_weight + negatives * 1.0
        print(f"Classification weights shape before normalization: {cls_weights.shape}")
        print(f"Positive normalizer shape: {pos_normalizer.shape}")
        
        cls_weights /= torch.clamp(pos_normalizer, min=1.0)
        print(f"Classification weights shape after normalization: {cls_weights.shape}")

        # Calculate focal loss
        cls_loss = sigmoid_focal_loss(cls_preds, cls_labls, weights=cls_weights, **self.cls)
        print(f"Classification loss shape before reduction: {cls_loss.shape}")
        
        cls_loss = cls_loss.sum() * self.cls['weight'] / batch_size
        print(f"Final classification loss value: {cls_loss.item()}")
        ...
        # Direction loss
        if self.dir:
            print("=== Computing direction loss ===")
            ...
            print(f"Direction loss: {dir_loss.item()}")

        # IoU loss
        if self.iou:
            print("=== Computing IoU loss ===")
            ...
            iou_pos_targets = self.iou_loss_func(boxes3d_pred.float()[:, [0, 1, 2, 5, 4, 3, 6]],
    ...
    def get_direction_target(self, reg_targets):
        ...
        Returns:
            dir_targets:
                ...
                NUM_BIN = 2
        """
        print(self.dir.keys(), self.dir['args'].keys())
